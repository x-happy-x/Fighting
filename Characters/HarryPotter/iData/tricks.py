...
class Attacking:
    ...
    def Punch(self):
        if self.MY.PROPERTY['Anim']['Punch.len']>self.MY.STATE_HS>=1:
            ...
            if self.at.colliderect(self.MY.Enemy.PROPERTY['BodyRect']) and (self.MY.Enemy.STATE_LIVE != self.MY.Enemy.DEAD_STATE):
        ...
        if self.MY.PROPERTY['Anim']['Kick.len']>self.MY.STATE_FS>=1:
            ...
            if self.at.colliderect(self.MY.Enemy.PROPERTY['BodyRect']) and (self.MY.Enemy.STATE_LIVE != self.MY.Enemy.DEAD_STATE):
                if (self.MY.Enemy.PROPERTY['Healf']-self.MY.PROPERTY['Force']//20*self.JutsuEnergy>=0):
                    ...
                    self.MY.Enemy.PROPERTY['Healf']=0
            if pg.key.get_pressed()[self.MY.PROPERTY['ControlKeys']['Kick']] and self.MY.PROPERTY['Energy']>0:
    ...
    def Rasengun(self):
        if self.MY.PROPERTY['Anim']['Kick.len']>self.MY.STATE_FS>=1:
            ...
            if self.at.colliderect(self.MY.Enemy.PROPERTY['BodyRect']) and (self.MY.Enemy.STATE_LIVE != self.MY.Enemy.DEAD_STATE):
                ...
                self.MY.AnimSpeed[3] =- self.MY.AnimSpeed[3]
                self.MY.STATE_FS += self.MY.AnimSpeed[3]
            if pg.key.get_pressed()[self.MY.PROPERTY['ControlKeys']['Kick']]:
